...
function normalizeEventShape(item = {}) {
  ...
  return {
    ...
    summary:
      ...
      normalizeMaybeEmptyText(item.description) ??
      '',
...
function normalizeOverview(overview, item) {
  ...
  const reportOverview =
    item.ai_report?.overview && typeof item.ai_report.overview === 'object'
      ? item.ai_report.overview
      : {}
  ...
  return {
    time:
      ...
      findFirstTimelineTime(item.timeline) ??
      normalizeDateTimeText(reportOverview.time) ??
      null,
    location:
      ...
      normalizeMaybeEmptyText(reportOverview.location) ??
      normalizeMaybeEmptyText(reportOverview.place) ??
      null,
    cause:
      normalizeMaybeEmptyText(normalizedOverview.cause) ??
      normalizeMaybeEmptyText(normalizedOverview.reason) ??
      normalizeMaybeEmptyText(reportOverview.cause) ??
      normalizeMaybeEmptyText(reportOverview.reason) ??
      null,
    persons: normalizePersons(
      normalizedOverview.persons ??
        normalizedOverview.people ??
        item.persons ??
        reportOverview.persons ??
        reportOverview.people,
    ),
  }
}

function findFirstTimelineTime(timeline) {
  if (!Array.isArray(timeline)) {
    return null
  }

  const firstTimelineItem = timeline.find((item) => normalizeDateTimeText(item?.time))
  return normalizeDateTimeText(firstTimelineItem?.time)
}
